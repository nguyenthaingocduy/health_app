<table class="table table-striped table-bordered" >
    ...
    <tr>
        <td><input type="checkbox" value="{{ $userCatalogue->id }}" class="checkboxItem input-checkbox"></td>
        <td>
           {{ $userCatalogue->name }}
        </td>
        {{-- <td class="text-center js-switch-{{ $userCatalogue->id }}"> 

            <input type="checkbox" value="{{ $userCatalogue->publish }}" class="js-switch status" data-field="publish" data-model="User" {{ ($user->publish == 1) ? 'checked' : '' }} data-modelId = {{ $userCatalogue->id }} />
        </td> --}}
